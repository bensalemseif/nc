...
export default function PromotionPageSettings(){
...
try{
    const response =await api.get("/settings/promotionPage");
    setPage(
     response.data
...
const handlerInputChange=async(e)=>{
 ...
 try{
    ...
    const response= await api.put('settings/promotionPage', {...Page}, {
})
showToast(
"success in updating Product promotion Page ","success"
);
      fetchPageList()
 }
      catch(err){
...
const handleFileInputChange = (event) => {
  ...
  const uploadImage = async (file) => {
    ...
    try {
    if(imagePath!==null){
      await api.delete(`/upload/delete-image/promotionPage/${Page._id}`, {
        method: 'delete',
    ...
    const response=  await api.post(`/upload/add/promotionPage/${Page._id}`, formData, {
        headers: {
 ...
 <div>
    ...
    <div class="my-3 h-12 px-10 flex items-center justify-between">
        <h1 class="font-medium text-2xl text-gray-800">Page d'acceuil</h1>
    </div>
